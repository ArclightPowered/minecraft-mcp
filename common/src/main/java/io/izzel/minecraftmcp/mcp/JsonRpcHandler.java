...
public final class JsonRpcHandler {
    ...
    public String handle(String requestJson) {
        ...
        try {
            ...
            switch (method) {
                case "initialize" ->
                    result = Map.of("protocolVersion", "2024-11-05", "serverInfo", Map.of("name", "minecraft-mcp", "version", "0.1.0"), "capabilities", Map.of("tools", Map.of(), "resources", Map.of()));
    ...
    private Object callTool(Map<String, Object> params) {
        ...
        try {
            ...
            return content(Json.stringify(result), false);
    ...
    private String response(Object id, Object result) {
        return Json.stringify(new LinkedHashMap<>(Map.of("jsonrpc", "2.0", "id", id == null ? 0 : id, "result", result)));
    }
    ...
    private String error(Object id, int code, String message) {
        ...
        r.put("jsonrpc", "2.0");
        r.put("id", id == null ? 0 : id);
        r.put("error", e);
        return Json.stringify(r);
    }

    static final class McpException extends RuntimeException {
        final int code;
