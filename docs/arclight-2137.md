...
- send normal client swing packets via `mc.client.player.swing`;
- observe world/player state;
- observe passenger/vehicle state via `mc.client.vehicle.state`.

`mc.client.player.swing` intentionally calls the vanilla client `LocalPlayer#swing(...)` path,
...
- `012_spawn_and_mount_vanilla_boat.json`: send vanilla server commands to summon a
  ...
  local test worlds created by `mc.client.world.join`, which enable cheats by
  default.
...
2. Either call `mc.client.connection.connect` directly, or copy `000_connect_external_server.json`
   and set its `address` to the target host:port.
3. Run the batch without assuming singleplayer-only tools. `mc.client.command.run` sends
...
- `mc.client.vehicle.state` changes from `isPassenger=true` to `isPassenger=false`, or
  the player otherwise leaves the controlled seat unexpectedly.

Expected fixed behavior:

- repeated `mc.client.player.swing` calls do not produce the Sable AABB abort stack through
